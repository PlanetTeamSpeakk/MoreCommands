...
public class ExceptionPacket extends AbstractPacket {
	...
	public void fromBytes(ByteBuf buf) {
		if (buf.readableBytes() != 0) try {
			packetClass = (Class<? extends AbstractPacket>) Class.forName(buf.readCharSequence(buf.readInt(), StandardCharsets.UTF_8).toString(), true, ClassLoader.getSystemClassLoader());
	...
	public void toBytes(ByteBuf buf) {
		try {
			...
			buf.writeCharSequence(packetClass.getName(), StandardCharsets.UTF_8);
