			...
			if (args.length == 0) Reference.sendCommandUsage(sender, usage);
				...
				if (args.length >= 3 && Reference.isBoolean(args[2]) && Boolean.parseBoolean(args[2]) || args.length < 3) {
					...
					try {
						...
						try {
							id = UUIDTypeAdapter.fromString(Reference.firstNonNull(new Gson().fromJson(Reference.getHTML("https://api.mojang.com/users/profiles/minecraft/" + args[0]), Map.class).get("id").toString(), ((EntityLivingBase) sender).getUniqueID().toString()));
