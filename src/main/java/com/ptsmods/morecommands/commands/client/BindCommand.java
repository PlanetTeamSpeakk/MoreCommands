...
public class BindCommand extends ClientCommand {
	...
	public void cRegister(CommandDispatcher<ClientCommandSource> dispatcher) {
		dispatcher.register(cLiteral("bind").then(cLiteral("set").then(cArgument("key", new KeyArgumentType()).then(cArgument("msg", StringArgumentType.greedyString()).executes(ctx -> {
			String key = MoreCommandsClient.getKeyForKeyCode(ctx.getArgument("key", Integer.class));
			...
			return 0;
		})))).then(cLiteral("remove").then(cArgument("key", new KeyArgumentType()).executes(ctx -> {
			String key = MoreCommandsClient.getKeyForKeyCode(ctx.getArgument("key", Integer.class));
			...
			return 0;
		})).then(cLiteral("listkeys").executes(ctx -> {
			sendMsg("The following keys are to your disposal: " + joinNicely(MoreCommandsClient.getKeys()) + ".");
			return MoreCommandsClient.getKeys().size();
		})).then(cLiteral("record").executes(ctx -> executeRecord(2)).then(cArgument("amount", IntegerArgumentType.integer(1)).executes(ctx -> executeRecord(ctx.getArgument("amount", Integer.class))))));
	}
