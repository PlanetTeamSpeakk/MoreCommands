	...
	public static class CommandfakePlayer extends com.ptsmods.morecommands.miscellaneous.CommandBase {
		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
			...
			if (args.length == 0) Reference.sendCommandUsage(sender, usage);
				...
				if (!tag.hasNoTags() && !tag.getCompoundTag("ForgeCaps").hasNoTags() && !tag.getCompoundTag("ForgeCaps").getCompoundTag(new ResourceLocation(Reference.MOD_ID, "fakeplayer").toString()).getBoolean("isFake")) {
				...
				if (player.dimension != sender.getEntityWorld().provider.getDimension()) server.getPlayerList().transferPlayerToDimension(player, sender.getEntityWorld().provider.getDimension(), new Teleporter((WorldServer) sender.getEntityWorld()) {
					@Override
				...
				if (args.length == 0) Reference.sendCommandUsage(sender, "/fakeplayer kick <name> Kick a player from the server if and only if it is fake.");
					...
					if (player.getCapability(FPProvider.fpCap, null).isFake) {
						player.connection.disconnect("You have been kicked by " + sender.getName() + ".");
				...
				if (args.length == 0) Reference.sendCommandUsage(sender, "/fakeplayer delete <name/UUID> Deletes a fake player entirely, even from the disk.");
					...
					if (Reference.isUUID(username)) UUIDTypeAdapter.fromString(username);
						...
						for (File data : new File(sender.getEntityWorld().getSaveHandler().getWorldDirectory(), "playerdata/").listFiles()) {
							...
							if (!tag.hasNoTags() && !tag.getCompoundTag("ForgeCaps").hasNoTags() && tag.getCompoundTag("ForgeCaps").getString(new ResourceLocation(Reference.MOD_ID, "playername").toString()).equals(username)) {
					...
					if (id == null) Reference.sendMessage(sender, TextFormatting.RED + "No UUID belonging to the given username could be found.");
						...
						if (!tag.hasNoTags() && !tag.getCompoundTag("ForgeCaps").hasNoTags() && tag.getCompoundTag("ForgeCaps").getCompoundTag(new ResourceLocation(Reference.MOD_ID, "fakeplayer").toString()).getBoolean("isFake")) if (new File(sender.getEntityWorld().getSaveHandler().getWorldDirectory(), "playerdata/" + id.toString() + ".dat").delete()) Reference.sendMessage(sender, "The fake player '" + tag.getCompoundTag("ForgeCaps").getCompoundTag(new ResourceLocation(Reference.MOD_ID, "fakeplayer").toString()).getString("name") + "' has been fully deleted.");
		...
		public static NBTTagCompound getPlayerData(File worldDir, UUID id) throws IOException {
			return worldDir == null || id == null ? new NBTTagCompound() : Reference.firstNonNull(CompressedStreamTools.readCompressed(new File(worldDir, "playerdata/" + id.toString() + ".dat").toURI().toURL().openStream()), new NBTTagCompound());
		}
