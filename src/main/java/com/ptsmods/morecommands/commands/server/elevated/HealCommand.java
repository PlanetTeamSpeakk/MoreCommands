...
public class HealCommand extends Command {
    ...
    public void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.getRoot().addChild(MoreCommands.createAlias("feed", dispatcher.register(literal("heal").requires(IS_OP).executes(ctx -> execute(ctx.getSource().getPlayer())).then(argument("players", EntityArgumentType.players()).executes(ctx -> {
