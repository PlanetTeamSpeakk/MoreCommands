...
public class SignCommand extends Command {
	...
	public void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(literalReq("sign").executes(ctx -> {
			...
			if (CompatHolder.getCompat().tagContains(new Identifier("minecraft:signs"), ctx.getSource().getWorld().getBlockState(result.getBlockPos()).getBlock()) && be instanceof SignBlockEntity) {
				SignBlockEntity sbe = (SignBlockEntity) be;
				((MixinSignBlockEntityAccessor) sbe).setEditable(true);
				CompatHolder.getCompat().setSignEditor(sbe, ctx.getSource().getPlayer());
