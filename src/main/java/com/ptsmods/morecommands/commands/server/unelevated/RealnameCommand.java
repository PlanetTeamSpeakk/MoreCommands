	...
	public void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(literalReq("realname").then(argument("query", StringArgumentType.word()).executes(ctx -> {
			...
			for (ServerPlayerEntity player : ctx.getSource().getServer().getPlayerManager().getPlayerList())
				if (MoreCommands.textToString(MoreCommands.getNickname(player), null, false).toLowerCase().contains(query) ||
						MoreCommands.textToString(player.getName(), null, true).toLowerCase().contains(query))
					players.add(player);
