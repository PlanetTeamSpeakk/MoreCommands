...
public class NicknameCommand extends Command {
	...
	private int execute(CommandContext<ServerCommandSource> ctx, String nickname, ServerPlayerEntity player) throws CommandSyntaxException {
		...
		String stripped = nickname == null ? null : Objects.requireNonNull(MoreCommands.stripFormattings(
				FabricLoader.getInstance().isModLoaded("placeholder-api") ? PlaceholderAPI.PLACEHOLDER_PATTERN.matcher(nickname).replaceAll("") : nickname));
		...
		if (nickname != null && stripped.length() > ctx.getSource().getWorld().getGameRules().getInt(MoreGameRules.nicknameLimitRule) && !isOp(ctx))
			...
			player.getDataTracker().set(DataTrackerHelper.NICKNAME, nickname == null ? Optional.empty() : Optional.of(new LiteralText(nickname)));
