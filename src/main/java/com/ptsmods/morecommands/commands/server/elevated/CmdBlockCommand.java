...
public class CmdBlockCommand extends Command {
    ...
    private int giveCmdBlock(CommandContext<ServerCommandSource> ctx, PlayerEntity player, int amount) throws CommandSyntaxException {
        ...
        sendMsg(ctx, (player == ctx.getSource().getPlayer() ? "You have" : SF + MoreCommands.textToString(player.getDisplayName(), SS) + Formatting.RESET + " has") + " been given " + SF + amount + " command block" + (amount == 1 ? "" : "s") + Formatting.RESET + ".");
        return amount;
    }
