			...
			if (args.length < 2 || !Lists.newArrayList("setcolour", "setcolor", "settype", "add", "remove").contains(args[0])) Reference.sendCommandUsage(sender, usage);
				...
				if (held.getItem() != Items.POTIONITEM && held.getItem() != Items.TIPPED_ARROW) Reference.sendMessage(sender, "You must be holding a potion item or a tipped arrow item for this command to work.");
					...
					if (!Reference.isInteger(args[1], 16) && !Reference.isInteger(args[1].substring(1), 16)) Reference.sendMessage(sender, TextFormatting.RED + "The given colour was invalid.");
						...
						NBTTagCompound nbt = Reference.firstNonNull(held.getTagCompound(), new NBTTagCompound());
					...
					if (effect == null) Reference.sendMessage(sender, TextFormatting.RED + "The given effect could not be found.");
						...
						NBTTagList oldTagList = Reference.firstNonNull(held.getTagCompound(), new NBTTagCompound()).getTagList("CustomPotionEffects", 10).copy();
						PotionUtils.appendEffects(held, Lists.newArrayList(new PotionEffect(effect, duration, amplifier, ambient, showParticles)));
						NBTTagList newTagList = Reference.firstNonNull(held.getTagCompound(), new NBTTagCompound()).getTagList("CustomPotionEffects", 10);
					...
					NBTTagList list = Reference.firstNonNull(held.getTagCompound(), new NBTTagCompound()).getTagList("CustomPotionEffects", 10);
