											...
											for (int i3 = position.getX() - l2; i3 <= position.getX() + l2; ++i3) {
												...
												for (int k1 = position.getZ() - l2; k1 <= position.getZ() + l2; ++k1) {
													...
													if (Math.abs(j1) != l2 || Math.abs(l1) != l2 || rand.nextInt(2) != 0 && k2 != 0) {
														...
														if (state2.getBlock().isAir(state2, worldIn, blockpos) || state2.getBlock().isAir(state2, worldIn, blockpos)) setBlockAndNotifyAdequately(worldIn, blockpos, Blocks.LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.BIRCH).withProperty(BlockOldLeaf.CHECK_DECAY, Boolean.valueOf(false)));
													}
												}
											}
										}
											...
											if (rand.nextInt(5) == 0 && i > 5) for (int l3 = 0; l3 < 2; ++l3)
												for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
													if (rand.nextInt(4 - l3) == 0) {
														...
														setBlockAndNotifyAdequately(worldIn, position.add(enumfacing1.getXOffset(), i - 5 + l3, enumfacing1.getZOffset()), Blocks.COCOA.getDefaultState().withProperty(BlockCocoa.AGE, rand.nextInt(3)).withProperty(BlockCocoa.FACING, enumfacing));
													}
										}
										return true;
									} else return false;
