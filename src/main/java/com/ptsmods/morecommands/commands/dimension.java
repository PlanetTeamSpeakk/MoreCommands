	...
	public static class Commanddimension extends com.ptsmods.morecommands.miscellaneous.CommandBase {
		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
			if (args.length == 0 || !Reference.isInteger(args[0]) && !args[0].equals("list")) Reference.sendCommandUsage(sender, usage);
				...
				if (server.getWorld(index) == null) Reference.sendMessage(sender, TextFormatting.RED + "A world with an id of", index, "could not be found.");
				else if (sender instanceof EntityPlayerMP) server.getPlayerList().transferPlayerToDimension((EntityPlayerMP) sender, index, (world1, entity, yaw) -> {});
				else if (sender instanceof Entity) server.getPlayerList().transferEntityToWorld((Entity) sender, sender.getEntityWorld().provider.getDimension(), (WorldServer) sender.getEntityWorld(), server.getWorld(index), (world1, entity, yaw) -> {});
