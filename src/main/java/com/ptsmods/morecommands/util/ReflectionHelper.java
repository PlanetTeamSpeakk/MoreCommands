...
public class ReflectionHelper {
	...
	public static Field getYarnField(Class<?> clazz, String yarnName, String name) {
		Field f = MoreObjects.firstNonNull(getField(clazz, yarnName), getField(clazz, name));
		if (f != null) {
			cachedFields.get(clazz).put(yarnName, f);
			cachedFields.get(clazz).put(name, f);
		}
		return f;
	}
	...
	public static Field getField(Class<?> clazz, String field) {
		...
		if (!f.isAccessible()) f.setAccessible(true);
		cachedFields.computeIfAbsent(clazz, clazz0 -> new HashMap<>()).put(field, f);
		return f;
	}
