...
public class ServerEventHandler extends EventHandler {
	...
	public void onServerConnectionFromClient(ServerConnectionFromClientEvent event) {
		if (event.getManager().channel().pipeline().get("packet_handler") != null) {
			String s = "packet_handler";
			while (event.getManager().channel().pipeline().get(s) != null)
				s = RandomStringUtils.randomAlphanumeric(64);
			event.getManager().channel().pipeline().addBefore("packet_handler", s, new NetHandler(Side.SERVER));
		}
	}
