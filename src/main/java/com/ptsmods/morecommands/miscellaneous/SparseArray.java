...
/**
 * <code>SparseArray</code> maps integers to Objects and, unlike a normal array
 * of Objects,
 * its indices can contain gaps. <code>SparseArray</code> is intended to be more
 * memory-efficient
 * than a
 * <a href="/reference/java/util/HashMap"><code>HashMap</code></a>, because it
 * avoids
 ...
 * and deleting entries in the array. For containers holding up to hundreds of
 * items,
 ...
 * removing
 * keys: instead of compacting its array immediately, it leaves the removed
 * entry marked
 * as deleted. The entry can then be re-used for the same key or compacted later
 * in
 * a single garbage collection of all removed entries. This garbage collection
 * must be performed whenever the array needs to be grown, or when the map size
 * or
...
public class SparseArray<E> implements Cloneable {
	...
	/**
	 * @hide
	 * 		Removes the mapping from the specified key, if there was any, returning
	 *       the old value.
	 */
	 ...
	 *
	 * <p>
	 * For indices outside of the range <code>0...size()-1</code>,
	...
	public int indexOfValueByValue(E value) {
		...
		for (int i = 0; i < mSize; i++)
			...
			} else if (value.equals(mValues[i])) return i;
		return -1;
	}
	...
	// From android.util.ContainerHelpers
	...
	// From com.android.internal.util.GrowingArrayUtils
	...
	public static <T> T[] insert(T[] array, int currentSize, int index, T element) {
		...
		T[] newArray = Arrays.copyOf(new Object[0], growSize(currentSize), (Class<T[]>) array.getClass());
		System.arraycopy(array, 0, newArray, 0, index);
		newArray[index] = element;
		System.arraycopy(array, index, newArray, index + 1, array.length - index);
		return newArray;
