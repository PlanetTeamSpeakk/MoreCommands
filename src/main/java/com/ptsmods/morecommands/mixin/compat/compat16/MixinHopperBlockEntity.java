...
public class MixinHopperBlockEntity {
    ...
    private void insertAndExtract_setCooldown(HopperBlockEntity thiz, int cooldown, Supplier<Boolean> extractMethod) {
        transferCooldown = Objects.requireNonNull(thiz.getWorld()).getGameRules().getInt(MoreCommands.hopperTransferCooldownRule);
    }
    ...
    private ItemStack insert_removeStack(HopperBlockEntity thiz, int slot, int amount) {
        return thiz.removeStack(slot, Objects.requireNonNull(thiz.getWorld()).getGameRules().getInt(MoreCommands.hopperTransferRateRule));
    }
}
