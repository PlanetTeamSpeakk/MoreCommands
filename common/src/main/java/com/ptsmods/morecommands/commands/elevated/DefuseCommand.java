...
public class DefuseCommand extends Command {
    ...
    private int defuse(CommandContext<CommandSourceStack> ctx, Collection<PrimedTnt> entities) {
        for (PrimedTnt tnt : entities) {
            tnt.kill();
            tnt.getCommandSenderWorld().addFreshEntity(new ItemEntity(tnt.getCommandSenderWorld(), tnt.getX(), tnt.getY(), tnt.getZ(),
                    new ItemStack(Compat.get().<Item>getBuiltInRegistry("item").get(new ResourceLocation("minecraft:tnt")), 1)));
        }
        sendMsg(ctx, SF + String.valueOf(entities.size()) + " TNT entit" + (entities.size() == 1 ? "y" : "ies") + " " +
                DF + "ha" + (entities.size() == 1 ? "s" : "ve") + " been killed and " + (entities.size() == 1 ? "a " : "") +
                "TNT item" + (entities.size() == 1 ? "" : "s") + " ha" + (entities.size() == 1 ? "s" : "ve") + " been spawned in " +
                (entities.size() == 1 ? "its" : "their") + " place.");
